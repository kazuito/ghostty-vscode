...
export const GHOSTTY_CONFIG_SCHEMA: GhosttyConfigKey[] = [
  // Alpha Blending
  {
    key: 'alpha-blending',
...
export const GHOSTTY_CONFIG_MAP = new Map<string, GhosttyConfigKey>();
GHOSTTY_CONFIG_SCHEMA.forEach(key => {
  GHOSTTY_CONFIG_MAP.set(key.key, key);
});

export function getConfigKeyInfo(key: string): GhosttyConfigKey | undefined {
  return GHOSTTY_CONFIG_MAP.get(key);
}

export function getAllConfigKeys(): string[] {
  return GHOSTTY_CONFIG_SCHEMA.map(key => key.key);
}

export function getConfigKeysByType(type: string): GhosttyConfigKey[] {
  return GHOSTTY_CONFIG_SCHEMA.filter(key => key.valueType === type);
}

export function getConfigKeysByPlatform(platform: 'macos' | 'linux' | 'windows'): GhosttyConfigKey[] {
  return GHOSTTY_CONFIG_SCHEMA.filter(key => !key.platforms || key.platforms.includes(platform));
}