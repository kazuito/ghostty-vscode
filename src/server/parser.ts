...
export class GhosttyConfigParser {
  ...
  private parseValue(key: string, rawValue: string): ConfigValue {
  ...
  private parseBooleanValue(value: string): ConfigValue {
  ...
  private parseNumberValue(value: string): ConfigValue {
  ...
  private parseColorValue(value: string): ConfigValue {
  ...
  private parseEnumValue(value: string, enumValues: string[]): ConfigValue {
  ...
  private parseKeybindValue(value: string): ConfigValue {
    try {
  ...
  private parseKeybind(value: string): ParsedKeybind {
  ...
  private parseKeybindSequence(value: string): ParsedKeybind {
    const parts = value.split('=');
    if (parts.length !== 2) {
  ...
  private parseKeybindTrigger(trigger: string): KeybindTrigger {
    let remaining = trigger;
    let prefix: KeybindTrigger['prefix'] | undefined;
    let physical = false;

    // Check for prefixes
    const prefixMatch = remaining.match(/^(global|performable|unconsumed|all):/);
    if (prefixMatch) {
      prefix = prefixMatch[1] as KeybindTrigger['prefix'];
    ...
    for (const modifier of modifiers) {
      if (!GHOSTTY_MODIFIERS.includes(modifier as any)) {
        throw new Error(`Invalid modifier: ${modifier}. Valid modifiers: ${GHOSTTY_MODIFIERS.join(', ')}`);
      }
    }
  ...
  private parseKeybindAction(action: string): KeybindAction {
    const colonIndex = action.indexOf(':');
    if (colonIndex === -1) {
      // No parameter
      if (!GHOSTTY_KEYBIND_ACTIONS.includes(action as any)) {
        throw new Error(`Invalid keybind action: ${action}. Valid actions: ${GHOSTTY_KEYBIND_ACTIONS.join(', ')}`);
      }
      return { action };
    }
    ...
    if (!GHOSTTY_KEYBIND_ACTIONS.includes(actionName as any)) {
      throw new Error(`Invalid keybind action: ${actionName}. Valid actions: ${GHOSTTY_KEYBIND_ACTIONS.join(', ')}`);
    }

    return { action: actionName, parameter };
  }

  private parseThemeValue(value: string): ConfigValue {
  ...
  private parsePathValue(value: string): ConfigValue {
  ...
  private parsePercentageValue(value: string): ConfigValue {
...
export function parseGhosttyConfig(content: string): ParsedConfig {
