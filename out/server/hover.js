...
function getHoverInfo(text, offset, position) {
...
function getKeyHover(key, keyStart, keyEnd, line) {
    const keyInfo = (0, schema_1.getConfigKeyInfo)(key);
    if (!keyInfo) {
        return undefined;
    }
    const range = node_1.Range.create(line, keyStart, line, keyEnd);
...
function getValueHover(key, value, valueStart, valueEnd, line, character) {
    const keyInfo = (0, schema_1.getConfigKeyInfo)(key);
    if (!keyInfo) {
        return undefined;
    }
    const range = node_1.Range.create(line, valueStart, line, valueEnd);
    let documentation = '';
...
function getBooleanValueHover(value) {
...
function getColorValueHover(value) {
...
function getEnumValueHover(value, enumValues) {
...
function getThemeValueHover(value) {
...
function getKeybindValueHover(value) {
    try {
        ...
        if (types_1.GHOSTTY_KEYBIND_ACTIONS.includes(actionName)) {
            ...
            documentation += `**Invalid action:** \`${actionName}\`\n\nValid actions: ${types_1.GHOSTTY_KEYBIND_ACTIONS.join(', ')}`;
        }
...
function getNumberValueHover(value) {
...
function getPercentageValueHover(value) {
...
function getGenericValueHover(value, keyInfo) {
...
function formatKeyDocumentation(keyInfo) {
    ...
    if (keyInfo.enumValues && keyInfo.enumValues.length > 0) {
        doc += `\n\n**Valid Values:**\n${keyInfo.enumValues.map((val) => `- \`${val}\``).join('\n')}`;
    }
